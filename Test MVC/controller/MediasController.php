...
class MediasController extends Controller {
	...
	function admin_index($id = null){
		...
		if($this->request->data && !empty($_FILES['file']['name'])){
			if(strpos($_FILES['file']['type'], 'image') !== false){
				...
				$this->Session->setFlash("L'image a bien été chargée.");
			} else {
				$this->Form->errors['file'] = 'Le type de fichier n\'est pas correct.';
			}
		}
	...
	**/
	function admin_delete($id){
		...
		$this->Session->setFlash('Le fichier a bien été supprimé.');
